...
> Product supersession · 2026-09-05: references below to the existing right-side passage panel describe the verified implementation at that time. D14 now makes anchor-aware artifacts inside the left reading stream the target layout; the right side remains the map. See [inline reader artifacts](17-inline-reader-artifacts.md).
...
- The renderer uses a Z-up world: source progression rises vertically, X remains horizontal, and Y structure occupies depth. Stored semantic coordinates are unchanged.
...
- Drag to orbit in every view. Shift-drag pans. A 48° capture radius—60% wider than the original 30° threshold—attracts the camera; release settles it onto the nearest plane. Capture is directional: approaching a plane attracts and settles, while moving away follows the gesture without snapping back. Top-down capture depends only on tilt, then aligns to the nearest 90° heading on release. Continued input stops at the vertical limit without reflecting; reversing responds immediately. Reversed alignments remain the same projection. Zoom buttons and Reset view are available. Arrow keys on the scene orbit; arrows on a node traverse directed relations where available, otherwise source-list order. All labels are keyboard-focusable.
