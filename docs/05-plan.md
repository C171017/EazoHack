...
高亮不是条件增量；支持的 PDF 必须能形成稳定选文锚点。TXT 可作为条件增量。整书图未完成不得阻止段落辅助。3D 整书地图及三个规范投影已经确认，但生产渲染器、节点预算和演示完成度仍需排期。当前默认排除 OCR、复杂双栏/公式密集书、EPUB/DOCX、任意长书、截图/周期采集、模型生成可执行代码、登录/协作/云同步。图片生成仍在四路产品范围内，但真实服务深度待定。
