...
Explanation, Diagram, and Interactive panel use Vertex AI `gemini-3.8-flash`, configured through the server-only `GEMINI_MODEL` setting and existing Vertex credentials. The three methods share existing artifact validation and source anchoring. Illustration continues to use fal; whole-book analysis continues to use Vertex Gemini. Existing Inco and Vertex artifacts remain readable.
