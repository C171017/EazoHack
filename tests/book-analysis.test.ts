...
import { ModelRequestError } from '../src/server/book-analysis/vertex';
...
test('text pipeline saves exact coordinates, excludes rejected claims, resumes checkpoints and never publishes failed runs', async () => {
  ...
  const generate: Generate = async (_system, prompt, schema) => {
    ...
    else value = { themes: chunks.map((c, i) => ({ label: c.section, rationale: 'Test theme rationale.', nodeIds: [`n-${i + 1}-1`] })), identities: [{ label: 'Justice', nodeIds: ['n-1-1', 'n-2-1', 'n-3-1'] }], crossEdges: [{ source: 'n-2-1', target: 'n-1-1', type: 'defines', rationale: 'Definition.' }] };
  ...
  try {
    ...
    await writeFile(path.join(result.root, 'chunk-001.json'), '{"model":"fixture","value":{}}');
...
test('map pages retain source coordinates and filters can expose every occurrence', async () => {
  ...
  assert.equal(mapWindow(graph, { ...view, nodePage: 999 }, [0, 1]).page, 2);
