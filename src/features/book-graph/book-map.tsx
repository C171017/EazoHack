'use client';
import { Background, Controls, ReactFlow, type Viewport } from '@xyflow/react';
const nodes = [
  { id:'selection',position:{x:210,y:25},data:{label:'Selected passage'},style:{borderColor:'#839778',background:'#eef2eb'} },
...
export function BookMap({onViewportChange, viewport}: {onViewportChange:(viewport:Viewport)=>void; viewport:Viewport|null}) {
  return <div className="book-map relative h-full min-h-64 w-full">
    <ReactFlow nodes={nodes} edges={edges} fitView={!viewport} defaultViewport={viewport ?? undefined} onMoveEnd={(_,v)=>onViewportChange(v)} nodesDraggable={false} nodesConnectable={false} elementsSelectable={false} minZoom={0.3} maxZoom={1.5}>
      <Background color="#d4ddd2" gap={22} size={1}/><Controls showInteractive={false}/>
    </ReactFlow>
