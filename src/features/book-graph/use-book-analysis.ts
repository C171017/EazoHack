...
import { MapAccountRequiredError, startHostedAnalysis, type HostedStatus } from './hosted-analysis';
import { analysisRequest } from './analysis-request';
type AnalysisState = { status: 'starting' | 'running' | 'ready' | 'failed' | 'idle' | 'unavailable' | 'sign-in' | 'account-required'; stage: string; error?: string; key?: string; graph?: MapBootstrap; updatedAt?: number };
export function useBookAnalysis(bookId: string, preview: BookPreview, enabled: boolean, title = bookId, cloudSourceId?: string, cloudOwnerId?: string) {
  ...
  useEffect(() => {
    ...
    function failed(error:unknown) {
      ...
      if (error instanceof MapAccountRequiredError || status === 401) {
        setState({status: status === 401 ? 'sign-in' : 'account-required', stage: error instanceof Error ? error.message : 'Sign in to build your book map.'});
        return;
      }
    ...
    async function hosted() {
      try {
        const {source,owner}=await startHostedAnalysis({kind:'txt',title,bookId,preview},cloudSourceId,attempt>0,{owner:cloudOwnerId,allowUpload:!!cloudOwnerId || allowUpload,signal:controller.signal});
        async function check() {
          ...
          try {
            ...
            if(data.status==='idle') {
              setState({status:'idle',stage:'No map analysis is running. Retry to start your book map.'});return;
            }
    ...
    if (process.env.NODE_ENV === 'production' || cloudSourceId || cloudOwnerId) void hosted(); else void start();
    return()=>{active=false;controller.abort();clearTimeout(timer);};
  },[bookId,preview,enabled,attempt,title,cloudSourceId,cloudOwnerId,allowUpload]);
  return { ...state, addToAccount:()=>{setState({status:'starting',stage:'Saving your book and starting its map'});setAllowUpload(true);setAttempt(value=>value+1);}, retry:()=>{setState({status:'starting',stage:'Reconnecting to book analysis'});setAttempt(value=>value+1);} };
}
