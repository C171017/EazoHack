...
export function PdfReader({doc,fileHash,onSelection,onPageText,anchors=[],jumpTo=null,retryPage=null,onNotice}: {
  doc:PDFDocumentProxy;fileHash:string;onSelection:(value:PdfSelection)=>void;onPageText:(page:PageText)=>void;
  anchors?:SourceAnchor[];jumpTo?:{page:number;ticket:number}|null;retryPage?:{page:number;ticket:number}|null;onNotice:(message:string)=>void;
}) {
  ...
  useEffect(()=>{
    ...
    const resize=new ResizeObserver(entries=>setWidth(Math.max(160,Math.min(1100,entries[0].contentRect.width-32))));
  ...
  useEffect(()=>{const p=data.current.get(visible.page);if(p)onPageText(p);},[visible.page,onPageText]);
  useEffect(()=>{
    if(!jumpTo)return;
  ...
  const capture=async()=>{
    ...
    try {
      ...
      for(let i=start.page;i<=end.page;i++) {
        ...
        const selected=[...range.getClientRects()].filter(r=>r.width>0&&r.height>0&&r.top>=rect.top-1&&r.bottom<=rect.bottom+1&&r.left>=rect.left-1&&r.right<=rect.right+1);
  ...
  return <div className="pdf-reader-column">
    ...
    <div ref={root} className="pdf-scroll" tabIndex={0} aria-label="Continuous PDF reader" onMouseUp={()=>void capture()} onKeyUp={e=>{if(e.shiftKey)void capture();}}>
      {Array.from({length:doc.numPages},(_,index)=><div key={index} className="pdf-page" data-pdf-page={index} style={{width,height:width*(ratios[index]??1.65)}}>
        {visible.near.includes(index)?<PdfPage doc={doc} hash={fileHash} index={index} width={width} ocr={ocr} recognize={index===visible.page} retry={retryPage?.page===index?retryPage.ticket:0} highlight={anchors.find(a=>a.locators[0]?.kind==='pdf'&&a.locators[0].pageIndex===index)} onReady={ready} onSize={size}/>:<span className="pdf-placeholder" aria-hidden="true">Page {index+1}</span>}
      </div>)}
    </div>
  </div>;
}
