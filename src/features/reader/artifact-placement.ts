...
export const ArtifactPlacementSchema = z.object({
  ...
  order: z.number().int().nonnegative(), collapsed: z.boolean(),
}).strict();
...
export function placementsFor(artifacts: Artifact[], anchors: SourceAnchor[], saved: ArtifactPlacement[] = []): ArtifactPlacement[] {
  return artifacts.flatMap((artifact, order) => {
    ...
    return anchor && locator?.kind === 'txt' ? [{artifactId:artifact.id, selectionId:artifact.selectionId, anchorId:anchor.id, offset:locator.endOffset, mode:'block_after_selection' as const, order, collapsed:false}] : [];
  });
}
