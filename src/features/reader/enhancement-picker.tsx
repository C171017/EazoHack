...
export function EnhancementPicker({ position, busy, onChoose }: {
  ...
  useEffect(() => {
    ...
    const keyboard = (event: KeyboardEvent) => {
      ...
      // Option changes event.key on macOS, so match the physical number key.
      const option = enhancementOptions.find((_, index) => event.code === `Digit${index + 1}`);
      if (!option) return;
  ...
  return <div className={styles.picker} data-open={!!position} style={position ?? undefined}
    ...
    {enhancementOptions.map((option, index) => <button key={option.id} type="button"
      ...
      aria-label={option.label} aria-keyshortcuts={`Alt+${index + 1}`} aria-disabled={!option.route || busy}
