...
export function Workspace({preview}:{preview:BookPreview}) {
  ...
  async function save() {
    ...
    const repository=createWorkspaceRepository();
    try {
      const snapshot=await repository.save({schemaVersion:1,id:workspaceId,bookId:selection.bookId,selections:[selection],anchors,artifacts,interactionState,graphViewport:viewport,bookmarks:selection.anchorIds,savedAt:new Date().toISOString()});
  ...
  return <main className="flex min-h-screen flex-col lg:h-screen lg:overflow-hidden">
    <header className="flex h-18 shrink-0 items-center justify-between border-b border-line px-6 lg:px-9">
      <div className="flex items-center gap-8"><a href="/" className="font-reading text-3xl tracking-tight">eazo<span className="text-moss">.</span></a><div className="hidden border-l border-line pl-6 text-xs text-muted sm:block">A little further into the text</div></div>
    ...
    <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
      <section className="flex min-h-0 flex-col border-b border-line lg:w-[45%] lg:border-r lg:border-b-0" aria-label="Book reader">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-8 py-9 lg:px-12 xl:px-16">
          ...
          <div ref={reader} onMouseUp={captureSelection} onKeyUp={captureSelection} className="whitespace-pre-wrap font-reading text-[17px] leading-[1.95] selection:bg-highlight" data-testid="book-text">
            {validHighlight?<>{preview.text.slice(0,offset)}<mark className="rounded-sm bg-highlight text-ink">{activeAnchor.quote}</mark>{preview.text.slice(offset+activeAnchor.quote.length)}</>:preview.text}
          </div>
      ...
      <section className="flex min-h-0 flex-1 flex-col bg-[#f3f5f0]" aria-label="Exploration workspace">
        ...
        <div className="relative h-64 shrink-0 border-b border-line xl:h-[30vh]"><BookMap onViewportChange={setViewport} viewport={viewport}/><div className="absolute top-4 left-5 rounded-md bg-paper/95 px-3 py-2 text-[10px] text-muted">MOCK · route relationships, not a book knowledge graph</div></div>
        <div className="min-h-0 flex-1 overflow-y-auto p-5 xl:p-7">
          <div className="rounded-panel border border-line bg-paper p-5 shadow-panel">
            ...
            <div className="mt-5 border-t border-line pt-4"><p className="mb-3 text-[11px] text-muted">Mock test controls · routing and trigger policy remain open</p>
          ...
          {saved&&<div className="mt-5 flex items-center justify-between border-t border-line pt-4 text-[11px] text-muted"><span>One local reading checkpoint · {saved.artifacts.length} mock results</span><Button variant="ghost" onClick={()=>{setSelection(saved.selections[0]??null);setAnchors(saved.anchors);setArtifacts(saved.artifacts);setInteractionState(saved.interactionState);setRuns([]);setNotice('Opened the saved checkpoint.');reader.current?.scrollIntoView({block:'start',behavior:'smooth'});}}>Revisit ↗</Button></div>}
