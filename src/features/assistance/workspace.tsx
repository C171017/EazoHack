    ...
    <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
      ...
      <section className="exploration-space relative min-h-[960px] flex-1 overflow-hidden lg:min-h-0" aria-label="Exploration workspace">
        <div className="absolute inset-0">{graph.unavailable?<div className="mx-auto max-w-xl p-8 text-sm text-muted" role="status" aria-live="polite">
          ...
          <h2 className="mb-3 font-reading text-xl text-ink">{analyzeUploaded ? analysis.status === 'unavailable' ? 'Book map unavailable' : analysis.status === 'failed' ? 'Book map needs attention' : analysis.status === 'ready' ? 'Opening book map' : 'Building your book map' : cloudSourceId ? 'Book map pending' : bookId === 'hong-lou-meng' ? 'Explore a passage' : 'Map is unavailable'}</h2>
          ...
          {(analysis.status==='unavailable' || cloudSourceId) && <a className="mt-4 block underline" href="/cloud">Open Cloud library</a>}
          ...
          {!analyzeUploaded && bookId !== 'hong-lou-meng' && <button className="mt-4 underline" onClick={()=>window.location.reload()}>Check map again</button>}
