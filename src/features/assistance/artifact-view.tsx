...
export function ArtifactView({artifact,state,onStateChange}:{artifact:Artifact;state:State;onStateChange:(state:State)=>void}) {
  return <article className="rounded-panel border border-line bg-paper p-5"><div className="mb-4 flex justify-between text-[10px] uppercase tracking-widest"><span>{artifact.kind.replaceAll('_',' ')}</span><span className="text-moss">{artifact.provenance.label}</span></div>
