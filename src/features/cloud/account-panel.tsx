...
export default function AccountPanel({session}:{session:Session}) {
 ...
 const refresh=useCallback(async()=>{
  ...
  if(next.id!==session.id){window.location.replace(new URL('/?book=plato-republic&library=1',window.location.origin).href);return;}
 ...
 useEffect(()=>{
  ...
  const changed=()=>{generation.current++;window.location.replace(new URL('/?book=plato-republic&library=1',window.location.origin).href);};
 ...
 async function run(task:()=>Promise<void>){setBusy(true);setMessage('');try{await task();}catch(error){setMessage(error instanceof Error?error.message:'Please try again.');if(error instanceof CloudRequestError&&[401,403].includes(error.status)){setBooks([]);setJobs([]);setAccount(null);window.location.replace(new URL('/?book=plato-republic&library=1',window.location.origin).href);}}finally{setBusy(false);}}
 ...
 async function signOut(){await cloudRequest('logout',{},session.id??undefined);announceAccountChange();window.location.replace(new URL('/?book=plato-republic&library=1',window.location.origin).href);}
