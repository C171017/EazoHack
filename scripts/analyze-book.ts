...
async function main() {
  ...
  try {
    const graph=args.includes('--hierarchy-only')?GraphSchema.parse(JSON.parse(await readFile(path.join(outputRoot,'current-graph.json'),'utf8'))):(await analyzeText({ raw, bookId, outputRoot, model: analysisModel(), generate: generateStructured, log: console.log })).graph;
